...
DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "migration_id" = '20251107032639_messaging_notificationDeviceInstallation') THEN
    CREATE TABLE device_installations (
        device_installation_id uuid NOT NULL,
        profile_id uuid NOT NULL,
        device_id uuid NOT NULL,
        notification_push_token text NOT NULL,
        device_model text,
        time_zone text,
        is_deleted boolean NOT NULL,
        created_at timestamp with time zone NOT NULL DEFAULT (now()),
        modified_at timestamp with time zone NOT NULL DEFAULT (now()),
        CONSTRAINT pk_device_installations PRIMARY KEY (device_installation_id)
    );
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "migration_id" = '20251107032639_messaging_notificationDeviceInstallation') THEN
    INSERT INTO "__EFMigrationsHistory" (migration_id, product_version)
    VALUES ('20251107032639_messaging_notificationDeviceInstallation', '9.0.9');
    END IF;
END $EF$;
COMMIT;
